...
With the ramp on, a changed target does not snap: its *shown* value steps toward the fold by the triggering row's `increment` every 250 ms, retargeting live if the fold changes mid-ramp. Node ramps drag their derived sliders along proportionally; direct-morph targets ramp in slider space. Hide, unregister and direct morphs' bookkeeping stay instant; ramps pause with the game and snap to the fold on save load or actor unload. One coalesced apply per actor per tick.
