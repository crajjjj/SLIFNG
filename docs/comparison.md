...
| Mod events | All 21, including the `(modName, node)` vs `(node, modName)` argument-order swap on `SLIF_unregisterNode` | All 21 registered, same signatures, same swap kept verbatim. Only three have an observed sender, but an unregistered mod event fails silently rather than logging, so the whole surface is registered |
